...
func TestInstallDoesNotWriteCodeGraphInstructionBlockAfterGentleAIProvisioner(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(sandboxHome, ".codex", "agents", "dots-explorer.toml")); !os.IsNotExist(err) {
		t.Fatalf("install without Codex Spark tag wrote native explorer agent; stat err = %v", err)
	}
...
func TestInstallCodexSparkDelegationTagInstallsGuidance(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		if err != nil {
			t.Fatalf("install with Codex Spark tag did not write native agent %s: %v", tc.path, err)
		}

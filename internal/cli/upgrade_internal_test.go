...
func TestUpgradeContinuationArgsPreserveUpdateFlags(t *testing.T) {
	...
	got := upgradeContinuationArgs("custom.yaml", true, []string{"core", "work"}, []string{"core", "desktop"}, "/src", "/home", "/state", true, true, true, binPlan)
	want := []string{"dots", "upgrade", "--continue", "--file", "custom.yaml", "--profile", "core", "--profile", "work", "--tag", "core", "--tag", "desktop", "--source-root", "/src", "--home", "/home", "--state-root", "/state", "--yes", "--no-tui", "--output", "json", "--binary-channel", "homebrew", "--binary-current-version", "v0.18.0", "--binary-latest-version", "v0.19.0", "--binary-action", "homebrew-upgrade", "--binary-executable", "/bin/dots", "--binary-artifact", "dots_v0.19.0_darwin_arm64", "--binary-checksum", "abc123"}

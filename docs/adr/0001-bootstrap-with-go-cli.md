...
Distribution will start with GitHub Releases and GoReleaser in v1, publishing Release Artifacts and checksums for macOS amd64/arm64 and Linux amd64/arm64. The Bootstrapper must perform Checksum Verification before installing or executing the downloaded CLI. Homebrew Distribution is planned for v1.1 or phase 2, after the release artifact pipeline is stable. Initial versioning will use `v0.x`, with `v0.1.0` as the first usable release target.

The first MVP Configuration Set will include zsh, git, starship, and tmux. Neovim and larger application configurations are Deferred Configuration because they introduce plugin, language-server, and dependency complexity that can distract from proving the installer, backups, manifest selection, status, and conflict handling.

...
| `agents` | `core`, `agents` | Core dotfiles plus gentle-ai memory/context setup, cleanup, shared engineering skills, dots-owned global rules for supported agents, and Codex-only Spark subagent delegation guidance. Add `--tag sdd` for Gentle-AI SDD setup. gentle-ai persona Regenerated Content is cleaned up, not installed, by this repository. | Core Development Baseline via the `core` tag-scoped Dependency Set; `GitHub CLI` via the `agents` tag-scoped Dependency Set |

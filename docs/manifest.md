...
| `agents` | `core`, `agents` | Core dotfiles plus gentle-ai memory/context setup, cleanup, shared engineering skills, and dots-owned global rules for supported agents. Add `--tag codex-spark-delegation` for Codex-only Spark subagent delegation guidance, `--tag without-codex-spark-delegation` to remove it, and `--tag sdd` for Gentle-AI SDD setup. gentle-ai persona Regenerated Content is cleaned up, not installed, by this repository. | Core Development Baseline via the `core` tag-scoped Dependency Set; `GitHub CLI` via the `agents` tag-scoped Dependency Set |

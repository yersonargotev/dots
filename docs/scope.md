...
| Reversible uninstall | `dots uninstall` reverses an install using the Installation Metadata as the source of truth, computing an Uninstall Plan, previewing with `--dry-run`, and removing only dots-owned symlinks and unmodified copies. Drifted copies (`--force` to remove) and not-owned targets are skipped by default, `--restore-backups` returns targets to their pre-install Backup Set, and no removal escapes HOME. See [`docs/uninstall.md`](uninstall.md). |
...
| MVP Configuration Set | v1 proves the installer with the bounded `dots.yaml` manifest: shell, git, terminal, editor, and agent-tool config with explicit install strategies and dependencies. Agent-tool provisioning is driven through the closed provisioner allowlist (`gentle-ai`, `claude`, `codex`); dots versions the invocation, never the regenerated content (see [`docs/adr/0003-claude-plugin-provisioner.md`](adr/0003-claude-plugin-provisioner.md) and [`docs/adr/0004-codex-mcp-provisioner.md`](adr/0004-codex-mcp-provisioner.md)). OpenCode, which has no fragment-level MCP CLI, gets the same MCP server through a dots-owned config overlay merged via `OPENCODE_CONFIG` (see [`docs/adr/0005-opencode-mcp-config-overlay.md`](adr/0005-opencode-mcp-config-overlay.md)). Generated editor state remains out of scope. |

...
`yersonargotev/dots/skills/delegation` by the `agents` profile. Delegate
...
Delegation Preflight is required for non-trivial work. For Codex Spark, that
means checking the `dots:codex-spark-delegation` overlay plus both native custom
...
skill pointer. The Codex Spark block remains a separate
`<!-- dots:codex-spark-delegation -->` opt-in overlay that maps the skill's policy

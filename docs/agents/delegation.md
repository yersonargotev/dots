...
checking the model-neutral `dots:delegation` overlay in `~/.codex/AGENTS.md`.
The Codex overlay is portable global guidance: installing the profile is standing
user authorization for safe bounded Codex delegation across repositories, not only
for the dots repository. Codex still only spawns subagents when the active prompt,
workflow, or skill gives a direct subagent/parallel-agent/delegation ask, so
reusable workflows should say that explicitly instead of relying on a dots-only
workflow name. The skip reasons, including `tool-level permission required`, and
reporting format live in the `delegation` skill.

## Surface inventory

| Surface | Native delegation support | dots-managed artifact for this slice |
| --- | --- | --- |
| Codex | Official subagent workflows, built-in `explorer`/`worker`, and custom TOML agents under `~/.codex/agents/` or `.codex/agents/`; custom agents can set `model`, reasoning effort, sandbox mode, MCP servers, and instructions. | `codex-delegation` installs the `delegation` skill only for Codex and a generic overlay in `~/.codex/AGENTS.md`. Cleanup uses `without-codex-delegation`; the old Spark-named tags remain compatibility aliases. |
...
`<!-- dots:delegation -->` opt-in overlay that maps the skill's policy to Codex
explorer/worker usage instead of duplicating the full procedure. It must remain
repository-neutral because the same global Codex instructions are loaded in
non-dots projects. `--tag without-codex-delegation` removes the overlay while
preserving `dots:rules`, Engram, CodeGraph, Codex config, user-owned custom
agents, and unrelated agent baseline content.

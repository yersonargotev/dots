...
agent artifacts. Delegation is independent of the native Agent CLI Baseline:
the `codex-delegation` Profile installs it for Codex, while `agents` does not
install delegation skills or global instruction rules. Native artifacts are
only safe when the agent's official format and dots ownership/cleanup boundary
are both clear.

## Portable policy

The detailed procedure lives in the `delegation` skill installed from
`yersonargotev/dots/skills/delegation` by the `codex-delegation` Profile. Delegate
...
| Claude Code | Official Markdown subagents with YAML frontmatter under `.claude/agents/`, `~/.claude/agents/`, managed settings, `--agents`, or plugins; frontmatter supports `model`, tools, permission modes, and other controls. | No delegation artifact in the native Agent CLI Baseline. Existing marker-owned gentle-ai/dots rules are retired; unmarked instructions are preserved. |
| OpenCode | Official primary agents and subagents; custom agents can be JSON entries or Markdown files under `~/.config/opencode/agents/` or `.opencode/agents/`, including `mode`, `model`, `permission`, and prompt. | Native JSON preferences only; no delegation artifact. The optional web overlay composes independently. |
| VS Code Copilot / Copilot CLI | Official customization docs include VS Code Copilot instructions, skills, custom agents, language-model selection, and subagents; Copilot CLI documents a global custom-instructions file. | Native Copilot CLI preferences only; no delegation artifact. Marker-owned legacy instructions are retired without deleting shared files. |
| Antigravity | Official docs expose Antigravity rules/workflows and a subagents area, but this repository has not yet confirmed a stable file format and ownership boundary equivalent to Claude/OpenCode agent files. | Native Antigravity preferences only; no delegation artifact. |

## Cleanup and ownership

The detailed portable policy lives in the installed `delegation` skill. The
native `agents` Profile removes the retired `<!-- dots:rules -->` block while
preserving unmarked content. Codex delegation remains a separate
...
files, while preserving CodeGraph, Codex config, user-owned custom agents, and
unrelated native Agent CLI Baseline content.

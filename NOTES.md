...
- Codex delegation is opt-in through the narrow `codex-delegation` profile, not bundled into the broader `agents` baseline. The profile installs only the portable skill for Codex, the generic `dots:delegation` overlay, and the dots-owned native explorer/worker agents; Spark is the current model choice rather than the capability identity.
- Codex delegation has explicit cleanup through `without-codex-delegation`, removing only `<!-- dots:delegation -->...<!-- /dots:delegation -->` and the two dots-owned native agents while preserving `dots:rules`, Engram, CodeGraph, Codex config, and user-owned agents.
